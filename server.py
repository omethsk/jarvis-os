...
JARVIS_AI_SCHEMAS = {
    ...
    'site': (
        ...
        '1. Before writing any CSS, privately decide 2-3 adjectives that capture THIS brand specifically '
        '(not a generic category) - e.g. not just "coffee shop" but "rustic, warm, unhurried" vs "coffee '
        'shop" as "sleek, minimal, third-wave" would call for genuinely different palettes even though '
        'both are coffee shops.\n'
        '2. Translate those adjectives into color choices using color psychology, not a lookup table: warm/'
        'earthy adjectives suggest terracotta, rust, warm cream, olive; sleek/premium/tech suggests near-'
        'black or deep navy with one precise saturated accent (electric blue, acid green, amber); playful/'
        'youthful suggests high-saturation candy colors on a bold field; luxury/exclusive suggests a deep '
        'jewel tone (emerald, burgundy, midnight) with a metallic gold or champagne accent; organic/'
        'wellness suggests sage, clay, warm off-white with visible warmth. These are starting directions to '
        'reason from, not a menu to copy verbatim - the exact hues and whether the theme ends up dark or '
        'light must come from the specific adjectives you picked in step 1, not be the same choice every '
        'time.\n'
        '3. Whatever you land on, a background is FORBIDDEN if a designer would call it "gray" or "off-'
        'white" in a color picker - no #fff, #f5f5f5, #f7f7f7, #e7e7e7, #eeeeee, #f0f0f0, #fafafa, #f8f8f8, '
        'or any similarly washed-out/desaturated shade, flat or inside a gradient, on ANY background layer '
        '(body, section, card, nav) - every background must read as either genuinely dark (real lightness '
        'under ~20 percent) or genuinely, visibly tinted with color, never colorless.\n\n'
        'Pick real Google Fonts (never system-ui/Arial) for a distinctive display+body pairing matching the '
        'mood from step 1. Generous whitespace, large bold headlines (clamp() for fluid sizing), soft '
        'shadows, 12-20px rounded corners on cards and buttons.\n\n'
