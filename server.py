...
JARVIS_AI_SCHEMAS = {
    ...
    'site': (
        ...
        'premium site.\n\n'
        'COLOR (the single most common way an AI-generated site looks cheap - read this carefully): '
        'default to a DARK theme unless the subject specifically calls for bright/airy (a bakery, spa, '
        'children\'s brand, wellness product). Dark theme body background must have real lightness under '
        'roughly 20 percent - examples to use directly: #0a0a0f, #120a08, #0d1420, #1a0f1a - paired with '
        'ONE vivid, saturated accent color that repeats across buttons/links/highlights.\n'
        'If (and only if) the subject calls for a light background, it must be warm and visibly tinted, '
        'not gray - examples to use directly: #faf3e8 cream, #fdf0f5 blush, #f4f7ef sage, #fff8ee ivory. '
        'A light background is FORBIDDEN if a designer would call it "gray" or "off-white" in a color '
        'picker - that includes #fff, #f5f5f5, #f7f7f7, #e7e7e7, #eeeeee, #f0f0f0, #fafafa, #f8f8f8, and '
        'ANY similar washed-out shade, whether flat or used inside a gradient. This rule applies to every '
        'background layer (body, section, card, nav) - not just the outermost one.\n\n'
        'Pick real Google Fonts (never system-ui/Arial) for a distinctive display+body pairing matching the '
        'mood above. Generous whitespace, large bold headlines (clamp() for fluid sizing), soft shadows, '
        '12-20px rounded corners on cards and buttons.\n\n'
